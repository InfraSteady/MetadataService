...
const auth = async (req, res, next) => {
    try {
        ...
        const decode = jwt.verify(token, process.env.JWT_SECRET_KEY); // verity the token with the string
        const connection = await getConnection();
        const user = await connection.query(findUserById, [decode.user_id]); // verity whether a user exist with the id
        if (!user[0][0]) {
            throw new Error("");
        }
        req.user_id = user[0][0].id; // set the user_id with req, so that later we dont have to fetch user object again
        next();
    } catch (e) {
