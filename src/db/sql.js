...
const getAgentsByUserId = "SELECT a.* FROM `agents` a  WHERE a.`user_id` = ?";
const getServiceTestsByUserId =
    "SELECT stm.`id` as service_test_id, stm.`service_test_name`, stm.`service_test_endpoint`, stm.`service_test_request_type`, stm.`service_test_request_header`, stm.`service_test_request_body`, stm.`service_test_request_query_param`, stm.`interval`, stm.`degrade_response_time`, stm.`failed_response_time`, stm.`test_status`, stm.`test_muted` FROM `service_test_runner_data` stm JOIN `project_service_test_map` pst ON stm.`id` = pst.`service_test_id` JOIN `projects` p ON pst.`project_id` =  p.`id` WHERE p.`user_id` = ? AND p.`id` = ?";
